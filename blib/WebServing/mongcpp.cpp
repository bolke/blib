...
MongooseConnection* MongooseConnection::Clone(){
  return new MongooseConnection(m_conn);
}

time_t MongooseConnection::GetCreationTime(){
  return mg_birth_time(m_conn);
}
...
//-----------------------------------------------------------------------
MongooseRequest::MongooseRequest(struct mg_connection *conn):
  m_conn(conn)
{
}
...
{
    return std::string(getInfo()->request_method);
}
...
{
    return std::string(getInfo()->uri);
}

const std::string MongooseRequest::getHttpVersion() const
{
    return std::string(getInfo()->http_version);
}

/// use only for GET
const std::string MongooseRequest::getQueryString() const
{
    if (getInfo()->query_string != NULL)
      return std::string(getInfo()->query_string);
    else
      return "";
}
...
bool MongooseRequest::getVar(const std::string &name, std::string &output) const
{
    ...
    const char *qs = getInfo()->query_string;
...
void MongooseResponse::buildResponseText()
{
    if (m_addContentLen) {
        setHeaderValue("Content-length", toString(m_content.length()));
        addHeader();
        addTextLine("");
        addText(m_content);
    } else {
...
{
  m_content += text;
  m_addContentLen = (m_addContentLen || addLen);
}
                           ...
                           struct mg_connection *conn)
{
  ...
  MongooseServer* server = (MongooseServer* )info->user_data;
  return server->handleEvent(eventCode, conn);
}
                            ...
                            struct mg_connection *conn)
{
    ...
    std::auto_ptr<MongooseRequest> request(newRequest(conn));
...
MongooseRequest *MongooseServer::newRequest(struct mg_connection *conn)
{
    return new MongooseRequest(conn);
}
