...
EnumResult_t UploadRequestHandler::HandleRequest(mongoose::ServerHandlingEvent eventCode,mongoose::MongooseResponse &response,std::string uri,std::vector<std::pair<std::string,std::string>> &variables){
	EnumResult_t result=FAIL;
  if(eventCode==MG_UPLOAD){
    if(fi==NULL){
      for(size_t i=0;i<variables.size();i++){
        if(StringParser::Compare(variables[i].first,"filename")){
          fi=new FileInterface(StringParser::Append(tempFolder,""),std::fstream::binary|std::fstream::out);
          break;
        }
      }
    }
    if(fi!=NULL){
      char_t buffer[512];
      int32_t dataCnt=connection->read(buffer,512);
      if(dataCnt>0)
        fi->Push(*buffer,dataCnt);
    }
  }
	return result;
}

std::string UploadRequestHandler::GetUploadFolder(){
  return uploadFolder;
}

std::string UploadRequestHandler::GetTempFolder(){
  return tempFolder;
}

EnumResult_t UploadRequestHandler::SetUploadFolder(std::string uploadFolder){
  EnumResult_t result=FAIL;
...
EnumResult_t UploadRequestHandler::SetTempFolder(std::string tempFolder){
  EnumResult_t result=FAIL;
