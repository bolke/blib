...
FileInterface::FileInterface(std::string filename,std::ios_base::openmode filemode):Device(*(new ThreadLock(false))){
  if(lock->Lock()){
...
std::ios_base::openmode FileInterface::GetFilemode(){
  std::ios_base::openmode result=std::fstream::in;
  if(lock->Lock()){
    result=filemode;  
    lock->Unlock();
  }
...
EnumResult_t FileInterface::SetFilemode(std::ios_base::openmode filemode){
  EnumResult_t result=FAIL;
  if(lock->Lock()){
    bool reopen=false;
    if(IsOpen()){
