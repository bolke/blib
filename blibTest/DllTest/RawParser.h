...
public:
	static uint8_t GetUInt8_t(const char_t& buffer,const size_t start=0);												//put 1 bytes into uint8		return (char_t -> uint8_t)
	static int8_t GetInt8_t(const char_t& buffer,const size_t start=0);													//put 1 bytes into int8     return (char_t -> int8_t)
	static uint16_t GetUInt16_t(const char_t& buffer,const size_t start=0);											//put 2 bytes into uint16		return (char_t -> uint16_t)
	static int16_t GetInt16_t(const char_t& buffer,const size_t start=0);												//put 2 bytes into int16    return (char_t -> int16_t)
	static uint32_t GetUInt32_t(const char_t& buffer,const size_t start=0);											//put 4 bytes into uint32   return (char_t -> uint32_t)
	static int32_t GetInt32_t(const char_t& buffer,const size_t start=0);												//put 4 bytes into int32    return (char_t -> int32_t)
	static uint64_t GetUInt64_t(const char_t& buffer,const size_t start=0);											//put 8 bytes into uint64   return (char_t -> uint64_t)
	static int64_t GetInt64_t(const char_t& buffer,const size_t start=0);												//put 8 bytes into int64    return (char_t -> int64_t)
	static float32_t GetFloat32_t(const char_t& buffer,const size_t start=0);										//put 4 bytes into float32  return (char_t -> float32_t)
	static float64_t GetFloat64_t(const char_t& buffer,const size_t start=0);										//put 8 bytes into float64  return (char_t -> float64_t)
	...
	static EnumResult_t FromString(const std::string& source,const EnumVar_t type,void* target);				//convert value of type from string to type, inserted into a void pointer (string -> type -> void*)
	static EnumResult_t FromString(const std::string& source,const std::type_info* type,void* target);	//convert value of type from string to type, inserted into a void pointer (string -> type -> void*)
